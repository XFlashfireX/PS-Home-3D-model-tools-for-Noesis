...
def registerNoesisTypes():
   handle = noesis.register("Playstation Home", ".mdl")
...
class sampleFile: 
	...
	def loadAll(self, bs):
		...
		dpos = bs.tell ()
		pointer = bs.read(">i")[0]
		bs.seek(dpos + pointer + 8,NOESEEK_ABS)
		mypos = bs.tell()
		for i in range(0, numMeshes):
                   ...
                   #texture.name = rapi.getExtensionlessName(rapi.getLocalFileName(matIndex))
                   #print(texture)
                   ...
                   #rapi.rpgBindNormalBufferOfs(normals, noesis.RPGEODATA_FLOAT, nCT, 28)
